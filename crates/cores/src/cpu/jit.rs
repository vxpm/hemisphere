...
use lazuli::{
    ...
    gekko::{
        self, Cpu, DEQUANTIZATION_LUT, QUANTIZATION_LUT, QuantizedType,
...
const CTX_HOOKS: Hooks = {
    ...
    extern "sysv64-unwind" fn read_quantized(
        ...
        let scale = if ty != QuantizedType::F32 {
        ...
        let scaled = read * QUANTIZATION_LUT[(scale as usize) & 0b0011_1111];
        *value = scaled;
    ...
    extern "sysv64-unwind" fn write_quantized(
        ...
        let scaled = value * DEQUANTIZATION_LUT[(scale as usize) & 0b0011_1111];
        ...
        let block = match self.compiler.build(instructions) {
            ...
            Err(e) => match e {
                ...
                ppcjit::BuildError::Builder { source } => {
                    panic!("block builder error: {:?}", source)
                }
